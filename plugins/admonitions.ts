...
function normalizeOptions(providedOptions: Partial<AdmonitionOptions>): AdmonitionOptions {
...
const plugin: Plugin = function plugin(
	...
	function blockTokenizer(this: any, eat: any, value: string, silent: boolean) {
		...
		const now = eat.now()
		const [opening, keyword, title] = match as string[] as [string, string, string]
		const food = []
		const content = []
	...
	const Parser = (this.Parser as any).prototype
	if (!Parser.blockTokenizers) Parser.blockTokenizers = {}
