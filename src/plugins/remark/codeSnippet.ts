// Credit: Astro docs team
// Source: https://github.com/withastro/docs/blob/main/integrations/astro-code-snippets.ts#L220
...
export const codeSnippetAutoImport: Record<string, [string, string][]> = {
	"~/components/ui/CodeSnippet/CodeSnippet.astro": [["default", CodeSnippetTagname]],
}
...
// Utils
const isMDXFile = (file: VFile) => file.history[0].endsWith(".mdx")
interface NodeProps {
...
export const remarkCodeSnippet = (): Plugin<[], Root> => {
	const makeVisitor =
		...
		(code, index, parent) => {
			...
			if (title === undefined && index > 0) {
				...
				const matches =
					potentialFileName && FileNameCommentRegExp.exec(`// ${potentialFileName}`)
				if (matches) {
			...
			parent.children.splice(index, 1, codeSnippetWrapper as any)
		}
